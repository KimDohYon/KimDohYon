...
class WaveformDataset(torch.utils.data.Dataset):
    ...
    def _load_item(self, fileid: str, suffix: str,
        ...
        if self.dtype == np.complex128:
            ...
            # 32비트씩 자르기: 실수와 허수 부분을 각각 4바이트씩 분리
            waveform = np.asarray(waveform[0::2].astype(np.float32) + 1j * waveform[1::2].astype(np.float32), dtype=np.complex64)
...
def load_waveform(data_folder: str, num_samples: float, shuffle: bool) \
 -> torch.utils.data.Dataset:
    ...
    data = WaveformDataset(root=data_folder,
                                   dtype=np.float32,  # int16 -> float32로 변경
                                   shuffle=shuffle,
